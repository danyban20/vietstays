...
?>
        <form method="post" id="formBooking">
            ...
            ?>
            <h4 class="mb-2"><?php vv_e( 'Apartments' ); ?></h4>
                ...
                <table class="table table-borderless table-striped table-earning apartments_list">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center"><?php vv_e( 'Rooms' ); ?></th>
                            <th class="text-center"><?php vv_e( 'Max Guests' ); ?></th>
                    ...
                    <tbody>
                        ...
                        foreach($apartments as $apartment){
                            ...
                            foreach($districts as $district){
                                if($district['ID'] == gArrayItem($apartment,'district')) $district_name = $district['post_title'];
                            }
            ...
            <div class="row mb-2">
                <div class="col-6">
                    <h4><?php vv_e( 'Addons' ); ?> </h4>
                </div>
                <div class="col-6 text-right">
                    <button type="button" class="btn btn-info btn-sm btnAddItem" ><?php vv_e( 'New Addon' ); ?></button>
                </div>
            </div>
            <div class="table-responsive table--no-card m-b-30">
                <table class="table table-borderless table-striped table-earning items_list">
                    <thead>
                        <tr>
                            ...
                            <th class="text-right" style="width:100px"><?php vv_e( 'Qty' ); ?></th>
                            <th class="text-right" style="width:100px"><?php vv_e( 'Price' ); ?></th>
                    ...
                    <tbody>
                        ...
                        foreach($booking_items as $item){
                            ...
                            <tr class="booking_<?php echo $item['item_id'] ?>" >
                                ...
                                <td><a href="#" class="deleteAddon" ><i class="fa fa-trash"></i></a></td>
                            </tr>
                            <?php 
                        }
                        ?>
                ...
                <div class="modal-content">
                    <form method="post" id="newAddonForm" >
                        <div class="modal-header">
                            <h5 class="modal-title" id="newAddonLabel"><?php vv_e( 'New Addon' ); ?></h5>
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label><?php vv_e( 'Addon Type' ); ?></label>
                                <select name="addon_type" class="form-control form-control-sm" required >
                                    <option value="" selected ><?php echo esc_html( vv__( '-choose addon type-' ) ); ?></option>
                                    <option value="airport_pickup" data-name="<?php echo esc_attr( vv__( 'Airport Pick up' ) ); ?>" data-price="<?php echo vv_get_config('airport_pickup_cost') ?>" ><?php echo esc_html( vv__( 'Airport Pick up' ) ); ?> - $<?php echo number_format(vv_get_config('airport_pickup_cost'),2) ?></option>
                                    <option value="scooter_rental" data-name="<?php echo esc_attr( vv__( 'Scooter Rental' ) ); ?>" data-price="<?php echo vv_get_config('scooter_rental_cost') ?>"><?php echo esc_html( vv__( 'Scooter Rental' ) ); ?> - $<?php echo number_format(vv_get_config('scooter_rental_cost'),2) ?></option>
                                    <option value="food"><?php vv_e( 'Food' ); ?></option>
                                </select>
                            </div>
        ...
        <script type="text/javascript">
            ...
            function initDeleteAddon(){
                $('.deleteAddon').click(function (e){
                    if(confirm('<?php echo $vv_booking_apartments_js['delete_addon']; ?>')){
            ...
            $(document).ready(function (){
                ...
                $('#newAddonForm').submit(function (e){
                    ...
                    html += '<td><a href="#" class="deleteAddon"><i class="fa fa-trash"></i></a></td>';
                    html += '</tr>';
                ...
                $('.btnAddItem').click(function (){
                    ...
                    if(apartment_radio.length){                        
                        $.get('<?php echo vv_admin_url().'?action=get_apartment_foods_json&apartment_id=' ?>' + apartment_radio.val(), function (jsondata){
                            data = jsondata.data;
                            if(data.length > 0){
                                html = '';
                                ...
                                vvAlert('<?php echo $vv_booking_apartments_js['no_addon_available']; ?>');
                            }
                        });
                    }else{
                        vvAlert('<?php echo $vv_booking_apartments_js['choose_apartment']; ?>');
                    }
                });
